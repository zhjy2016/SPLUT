...
from Train_SPLUT_L import _baseq,BASEQ,SRNet,UPSCALE,VERSION

# USER PARAMS
MODEL_PATH = "./checkpoint/{}".format(VERSION)   # Trained SR net params
ITER=560000

LUT_NUM_LIST=[1,21,22,21,22,31,32,31,32]
KERNAL_LIST=[122,221,221,212,212,221,221,212,212]
img_bits=4
...
class SRNet_LUT(SRNet):
    ...
    def forward(self, x_in):
        ...
        elif LUT_NUM==21 and KERNAL==221:
            x_out = self.lut221(x_in)
        elif LUT_NUM==22 and KERNAL==221:
            x_out = self.lut221_2(x_in)
        elif LUT_NUM==21 and KERNAL==212:
            x_out = self.lut212(x_in)
        elif LUT_NUM==22 and KERNAL==212:
            x_out = self.lut212_2(x_in)
        elif LUT_NUM==31 and KERNAL==221:
            x_out = self.lut221_c12(x_in)
        elif LUT_NUM==32 and KERNAL==221:
            x_out = self.lut221_c12_2(x_in)
        elif LUT_NUM==31 and KERNAL==212:
            x_out = self.lut212_c34(x_in)
        elif LUT_NUM==32 and KERNAL==212:
            x_out = self.lut212_c34_2(x_in)
        return x_out
