...
class DirectoryIterator_DIV2K(Iterator):
    def __init__(self,
        ...
        srs = glob.glob(datadir+'/DIV2K_train_SR/*.png')
        srs.sort()
    ...
    def next(self):
        ...
        while (len(batch_blur) < self.out_batch_size):
            ...
            srs = self.sr_pngs[(current_index+i) % self.total_count]

            # check if png file is damaged
            try:
                B_ = _load_img_array(blurs) 
                S_ = _load_img_array(sharps) 
                SR_ = _load_img_array(srs) 
            except:
                print("File open error: {} {} {}".format(blurs, sharps,srs))
                raise
            ...
            for j in range(self.crop_per_image):
                ...
                SR = SR_[sh*self.r:(sh+self.crop_size)*self.r, sw*self.r:(sw+self.crop_size)*self.r]
        ...
        batch_sr = np.stack(batch_sr, 0).astype(np.float32)


        return batch_blur.transpose((0,3,1,2)), batch_sharp.transpose((0,3,1,2)), batch_sr.transpose((0,3,1,2))
