...
public class MainActivity extends AppCompatActivity {
    ...
    public static int layer23_pos(double[] ds, double[] out1, int x, int w, int h, int L2, int L3, String str) {
        if (str == "k221") {
            ...
            int a1 = clip_quan((float)ds[x] + (float)out1[x * 4] + (float)ds[top_pos] + (float)out1[top_pos * 4 + 2]);
            int bottom_pos = get_bottom_pos_all(x, w, h);
            int b1 = clip_quan((float)ds[bottom_pos] + (float)out1[bottom_pos * 4] + (float)ds[x] + (float)out1[x * 4 + 2]);
            int c1 = clip_quan((float)ds[x] + (float)out1[x * 4 + 1] + (float)ds[top_pos] + (float)out1[top_pos * 4 + 3]);
            int d1 = clip_quan((float)ds[bottom_pos] + (float)out1[bottom_pos * 4 + 1] + (float)ds[x] + (float)out1[x * 4 + 3]);
            return a1 * L3 + b1 * L2 + c1 * 16 + d1;
        } else {
            int left_pos = get_left_pos(x, w, h);
            int a1 = clip_quan((float)ds[x] + (float)out1[x * 4] + (float)ds[left_pos] + (float)out1[left_pos * 4 + 2]);
            int right_pos = get_right_pos(x, w, h);
            int b1 = clip_quan((float)ds[right_pos] + (float)out1[right_pos * 4] + (float)ds[x] + (float)out1[x * 4 + 2]);
            int c1 = clip_quan((float)ds[x] + (float)out1[x * 4 + 1] + (float)ds[left_pos] + (float)out1[left_pos * 4 + 3]);
            int d1 = clip_quan((float)ds[right_pos] + (float)out1[right_pos * 4 + 1] + (float)ds[x] + (float)out1[x * 4 + 3]);
            return a1 * L3 + b1 * L2 + c1 * 16 + d1;
        }
    }
    ...
    public DoubleStream lookup2_add(double xx, double[] ds, double[] out1, int w, int h, int L2, int L3, int N){
        ...
        for (int i = 0; i < 4; i++){
            out2[i] = out1[x*4+i] + (out21[i] + out22[i]) / 2;
        }
    ...
    public double[] lookupall(int[] isa, int w, int h, int N) {
        ...
        double[] ds = IntStream.range(0, 6 * N).parallel().asDoubleStream().map(x -> ((int)x / N) % 2 == 0 ? (float)is[(int)x] * 16f / 255f : (float)is[(int)x] / 255f).toArray();

        int L3 = 16 * 16 * 16;
        int L2 = 16 * 16;
        double[] out1 = IntStream.range(0, 6 * N).parallel().asDoubleStream().flatMap(x -> lookup1(x, isa, w, h, L2, L3, N)).toArray(); // 4*w*h
