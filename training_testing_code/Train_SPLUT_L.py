...
QQ=2**4
OUT_NUM=4
VERSION = "SPLUT_L_{}_{}".format(LR_G,OUT_NUM)
...
class SRNet(torch.nn.Module):
    def __init__(self, upscale=4):
        ...
        self.lut221_c12_2=nn.Sequential(
            nn.Conv2d(2,  64, [2,1], stride=1, padding=0, dilation=1),
            nn.GELU(),
            nn.Conv2d(64, 64, 1, stride=1, padding=0, dilation=1),
            nn.GELU(),
        ...
        self.lut212_c34_2=nn.Sequential(
            nn.Conv2d(2,  64, [1,2], stride=1, padding=0, dilation=1),
            nn.GELU(),
            nn.Conv2d(64, 64, 1, stride=1, padding=0, dilation=1),
            nn.GELU(),
    ...
    def forward(self, x_in):
        ...
        x221_2 = self.lut221_2(self.quant(F.pad(x[:,4 :6 ,:,:],(0,0,0,1), mode='reflect')+F.pad(x[:,6 :8 ,:,:],(0,0,1,0), mode='reflect')))
        x212   = self.lut212  (self.quant(F.pad(x[:,8 :10,:,:],(0,1,0,0), mode='reflect')+F.pad(x[:,10:12,:,:],(1,0,0,0), mode='reflect')))
        x212_2 = self.lut212_2(self.quant(F.pad(x[:,12:14,:,:],(0,1,0,0), mode='reflect')+F.pad(x[:,14:  ,:,:],(1,0,0,0), mode='reflect')))
        x=(x221+x221_2+x212+x212_2)/4.0+x

        x2   = self.lut221_c12  (self.quant(F.pad(x[:,0 :2 ,:,:],(0,0,0,1), mode='reflect')+F.pad(x[:,2 :4 ,:,:],(0,0,1,0), mode='reflect')))
        x2_2 = self.lut221_c12_2(self.quant(F.pad(x[:,4 :6 ,:,:],(0,0,0,1), mode='reflect')+F.pad(x[:,6 :8 ,:,:],(0,0,1,0), mode='reflect')))
        x3   = self.lut212_c34  (self.quant(F.pad(x[:,8 :10,:,:],(0,1,0,0), mode='reflect')+F.pad(x[:,10:12,:,:],(1,0,0,0), mode='reflect')))
        x3_2 = self.lut212_c34_2(self.quant(F.pad(x[:,12:14,:,:],(0,1,0,0), mode='reflect')+F.pad(x[:,14:  ,:,:],(1,0,0,0), mode='reflect')))
        x=(x2+x2_2+x3+x3_2)/4.0+x_temp[:,:,:H-1,:W-1]
